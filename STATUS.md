...
| Processセクションのトーン再調整 | **完了**。過多な説明と生成補助ビジュアルを外し、既存3画像と短いコピーで工程を見せる構成へ調整。下段はListen=打合せ、Imagine=模型検討、Build=現場確認に再整理 |
| Process英文コピーのホバー翻訳演出 | **完了**。英語コピーにマウスオーバーすると、同位置で日本語へなめらかに切り替わる演出を追加 |
| Peopleセクションの社員インタビュー再構成 | **完了**。マイナビ掲載インタビューをもとに、営業・デザイン設計・工務の3視点で構成。実写写真ではなく生成画像3点を使用 |
| People生成画像の人物差分調整 | **完了**。上田さんを女性デザイナーに修正し、吉田さんと加納さんは髪型・服装・職務シーンが分かれる画像へ差し替え |
| Peopleセクション表記調整 | **完了**。共通ヘッダーを正式ロゴ画像に戻し、若手社員インタビュー表現、氏名サイズ、所属部署・社歴併記を修正 |
